...
This files contains the class automaton and all the functions that are used to manipulate it.
"""
...
funcs = {}
# wrapper that chooses which two functions with the same name to execute depending if the automaton have empty word expression or not
# display error if function doesn't work with async automaton
    ...
    def wrapper(func):
        ...
        def wrapper2(*args, **kwargs):
            if args[0].isAsync == emptyWord:
                ...
                raise BadAutomata(
                    f"This function doesn't work {'without' if emptyWord else 'with'} async automaton !")
...
class Automata():
    # class that contains all the functions to manipulate automaton
        ...
        for state, properties in temp_states.items():
            for letter in self.language:
                if letter in properties.keys():
                    properties[letter] = list(
                        map(lambda x: stateName(x), properties[letter]))
        # pass each state to frozenset
        temp_states = {stateName(state): properties for state,
                       properties in temp_states.items()}
        self.states: dict[stateName,
                          dict[str, list[stateName]] | bool] = temp_states

    # function to get the initial state of the automaton, returns None if there is more than one
    ...
    # function to get the final states of the automaton
    ...
    # property to check if the automaton is async
    @property
    ...
    @isAsync.getter
    def isAsync(self) -> bool:
        # check if the automaton is async
        return "€" in self.language

    # property to get the initial state of the automaton
    @property
    ...
    # function to check if the automaton is standard
    ...
    def standardize(self) -> None:
        # standardize the automaton if not already standard
        if self.isStandard():
            raise BadAction("Your automaton is already standard")
        transition = {}
        ...
        # add the new initial state to the automaton
        self.states[stateName("i")] = state

    @emptyWordErrorWrapper(False)  # async automaton can't be complete
    ...
    def complete(self) -> None:
        # complete the automaton  for not async automaton if not already complete
        if self.isComplete() == True:
            raise BadAction("Your automaton is already complete")
        ...
        for properties in self.states.values():
            for letter in self.language:
                if letter not in properties.keys() or len(properties[letter]) == 0:
                    # add a transition to the trash state for each letter that doesn't have a transition
                    properties[letter] = [stateName("P")]

    def isDeterministic(self) -> bool:
        # check if the automaton is deterministic
        if self.isAsync:
            return False  # async automaton can't be deterministic
        if self.init_state == None:
            return False  # if there is more than one initial state
        for properties in self.states.values():
            for letter in self.language:
    ...
    @emptyWordErrorWrapper(False)  # normal automaton function
    def determinize(self) -> None:
        # determinize the automaton for not async automaton if not already deterministic
            ...
            for state, properties in self.states.items():
                for statepart in actual_state:
                    if statepart in state:
                        for letter in self.language:
                            if letter in properties.keys():
                                # append new arrival states to the list or create a new list
        ...
        if self.isDeterministic():
            raise BadAction("Your automaton is already deterministic")
        if self.init_state:  # if there is only one initial state
            ...
            init_states = stateNameSum(
                *[state for state, properties in self.states.items() if properties["start"]])
            self.states = determinizeRec(init_states, start=True)

    @emptyWordErrorWrapper(True)  # async automaton function
    def determinize(self) -> None:
        # determinize the automaton for async automaton if not already deterministic
        if self.isDeterministic():
            raise BadAction("Your automaton is already deterministic")
        # create list of new states
        # prime states are the epsilon closure of the states
        prime_states: dict[stateName, set[stateName]] = {}
        # get all the starting states
        starting_states = [
            state for state, properties in self.states.items() if properties["start"]]
        # create the new initial state
        prime_states[stateName(*[s.getPrime() for s in starting_states])
                     ] = self._computeEpsilonClosure(*starting_states)
        new_automaton = {}
        ...
        while prime_states:  # while there is still new states to process
            ...
            for letter in self.language:
                ...
                if len(next_states) > 0:
                    new_prime_state_name = stateName(
                        *[s.getPrime() for s in next_states])
                    new_state[letter] = [new_prime_state_name]
                    prime_states[new_prime_state_name] = self._computeEpsilonClosure(
                        *next_states)  # add the new state to the list of new states
            new_state["start"] = isInit
            # if one of the states is an end state, the new state is an end state
            new_state["end"] = any(self.states[s]["end"]
                                   for s in englobed_states)
            isInit = False  # the initial state has been added
            # add the new state to the automaton
    ...
    # async automaton can't be complete so we can't get the complement
    @emptyWordErrorWrapper(False)
    def complementary(self) -> None:
        if not self.isDeterministic():  # automaton need to be deterministic to obtain the complement
            raise BadAction(
                "Please determinize the automaton before making its complementary")
    ...
    def minimize(self) -> None:
        ...
        condition = []
        if not self.isDeterministic():  # automaton need to be deterministic to minimize it
            condition += ["determinize"]
        if not self.isComplete():  # automaton need to be complete to minimize it
            condition += ["complete"]
        if condition:
            raise BadAction(
                f"Please { ' and '.join(map(str,condition)) } the automaton before minimizing it!")

        # construct the initial partition (Θ0 = {F, NF})
        F = self.findFinalStates()  # final states
        NF = set(self.states.keys()) - F  # non-final states
        θcurrent: list[set:stateName] = [group for group in [
            F, NF] if group]  # exclude empty sets in θcurrent
        ...
        while θprev != θcurrent:  # while the partition is not stable
            ...
            groupNames = {stateName(f"({num})"): group for num,
                          group in enumerate(θcurrent)}
            subgroups: list[set[stateName]] = []

            for groupStateNames in θprev:  # for each group in the current partition
                groupStates: dict[stateName:dict[str:list[stateName] | bool]] = {
                    key: self.states[key] for key in groupStateNames}
                ...
                if len(groupStates) > 1:  # if the group isn't a singleton
                    ...
                    for state, properties in groupStates.items():
                        ...
                        for letter in self.language:
                            if letter in properties.keys():
                                new_transitions[letter] = [groupName for groupName, group in groupNames.items(
                                ) if properties[letter][0] in group]
                        ...
                        for state, properties in groupStates.items():
                            ...
                            for letter in self.language:
                                if letter in properties.keys():  # if the state has a transition for the letter
                                    line.append(
                                        ", ".join(map(str, properties[letter])))
                            table.append(line)
                        ...
                        for state in groupStates.keys():
                            ...
                            for letter in self.language:
                                partitionLine.append(
                                    partitionStates[state][letter][0])
                            partitionTable.append(partitionLine)

                        table1 = tabulate(
                            table, headers=["State"]+self.language, tablefmt="rounded_grid")
                        table2 = tabulate(
                            partitionTable, headers=self.language, tablefmt="rounded_grid")

                        # print title in bold, red and underlined
                        print(
                            f" \033[1m\033[4m\033[31mIteration θ {i} :\033[0m")
                        print(tabulate([[key, ", ".join(map(str, val))] for key, val in groupNames.items(
                        )], headers=['Group names', 'States contained'], tablefmt="rounded_grid"), end="\n")
                        print(tabulate([[table1, table2]], headers=[
                              'Original', f'Under θ {i}'], tablefmt="rounded_grid"), end="\n\n\n")
                    ...
                    # this loop is to group the states in the current group
            ...
            θcurrent = subgroups
            i += 1
        # if we obtain the same number of groups as states then the automaton is already minimal
            ...
            for letter in self.language:
                for state in group:
                    if letter in self.states[state]:
                        new_state[letter] = [groupName for groupName, group in groupNames.items(
                        ) if self.states[state][letter][0] in group]
                        break
            group_name = [groupName for groupName,
                          group in groupNames.items() if state in group][0]
            new_automaton[group_name] = new_state
        self.states = new_automaton  # replace the old automaton with the new one

    @emptyWordErrorWrapper(True)
    # cache for epsilon closure so that it is computed only once for each state
    ...
    def _computeNextStates(self, state, letter) -> set[stateName]:
        ...
        if letter in self.states[state]:
            # return the next states if there is a transition for the letter as a set
        ...
        def recognizeRec(state: str, word: str) -> bool:
            if word == "":  # if the word is empty, return true if the state is an end state
                ...
                if letter not in self.language:
                    # if the word contains a letter that is not in the language of the automaton, raise an exception
                    raise Exception(
                        "The word contains a letter that is not in the language of the automaton")
                if letter not in self.states[state].keys():
                    ...
                    for endState in self.states[state][letter]:
                        # if any of the next states can recognize the rest of the word, return true
                        return recognizeRec(endState, rest)
                    return False  # if none of the next states can recognize the rest of the word, return false
        if self.init_state:
            # if there is an initial state, start from it
            return recognizeRec(self.init_state, word)
        else:
            init_states = [state for state, properties in self.states.items(
            ) if properties["start"]]  # else test from all the initial states
            # return true if any of the initial states can recognize the word
            return any(map(lambda state: recognizeRec(state, word), init_states))

    @emptyWordErrorWrapper(True)
    # function to recognize a word with the empty word expression in the automaton
    def recognize(self, word: str, sep='') -> bool:
        ...
        def recognizeRec(state: stateName, word: str) -> bool:
            if word == "":
                ...
                if letter not in self.language:
                    raise Exception(
                        f"The word contains a letter that is not in the language of the automaton : {letter}")
                if "€" in self.states[state]:
                    for endState in self.states[state]["€"]:
                        if recognizeRec(endState, word):
                            return True
